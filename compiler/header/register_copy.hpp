...
std::string register_copy(std::string & src, std::string & dest, eval_info_t & eval_info) {
    if (std::regex_search(src, number_regex)) {
        ...
        if (dst_reg.in_use) throw std::runtime_error("Register \"" + dest + "\" is already set when being copied to");
        ...
        try {
            ...
            if (!src_reg.in_use) throw std::runtime_error("Register \"" + src + "\" is not set when being copied");
            if (dst_reg.in_use) throw std::runtime_error("Register \"" + dest + "\" is already set when being copied to");
            dst_reg.in_use = true;
